...
export async function fetchAgentRuns(
  ...
  limit: number = 20,
  base: string = "",
): Promise<AgentRunsPayload> {
  const query = new URLSearchParams({ session_key: sessionKey, limit: String(limit) });
  return request<AgentRunsPayload>(
    `${base}/api/omniagent/runs?${query}`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
