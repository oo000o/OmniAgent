...
class GatewayHTTPHandler:
    ...
    def __init__(
        ...
        *,
        ...
        omniagent_run_store: RunStore | None = None,
        log: Any = logger,
    ) -> None:
        self.config = config
        ...
        self.omniagent_run_store = omniagent_run_store
    ...
    def _handle_omniagent_runs(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        ...
        raw_limit = _query_first(query, "limit") or "50"
        try:
            limit = int(raw_limit)
            runs = self.omniagent_run_store.list(session_key=session_key, limit=limit)
        except ValueError as exc:
            return _http_error(400, str(exc))
