...
# Optional local/container deployment template. Initialize the writable runtime
# config once, then preserve changes made by channels and the WebUI on restarts.
if [ -n "$NANOBOT_CONFIG_TEMPLATE" ]; then
    config="$dir/config.json"
    if [ ! -f "$NANOBOT_CONFIG_TEMPLATE" ]; then
        echo "[entrypoint] error: config template not found: $NANOBOT_CONFIG_TEMPLATE" >&2
        exit 1
    fi
    mkdir -p "$dir"
    if [ ! -f "$config" ]; then
