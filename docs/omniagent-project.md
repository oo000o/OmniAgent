...
报告保存在 `artifacts/evaluation/latest.json`。当前基线包含 92 条确定性工程用例，覆盖
检索、引用、RRF、任务幂等、错误参数拒绝、可恢复求职工作流、运行观测和故障注入；其中 12 条带相关性标注
的查询用于对比
...
每次知识检索还会输出结构化事件，包括检索模式、成功/降级/失败状态、端到端耗时、
返回条数、请求上限和候选集上限。事件刻意不记录查询正文与文档内容，避免把私有知识
写入日志。
...
- [x] `python -m evaluation.run` 达到 92/92，并输出三种检索策略的标准指标。
...
- 建设运行观测与自动化质量门禁，记录耗时、Token、工具调用、重试、错误码和最小 Trace；
  建立 92 条可复现离线评测，并接入 pytest、Docker 与 GitHub Actions。
